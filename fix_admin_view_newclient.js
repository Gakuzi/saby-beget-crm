...
const regex = /export function renderNewClientPage\(\) \{[\s\S]*?<\/script>\n<\/body><\/html>`/m;
if (!regex.test(code)) {
  console.log('Regex failed');
  process.exit(1);
}

const newFunction = `export function renderNewClientPage() {
  return \`<!DOCTYPE html>
<html lang="ru">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Добавление контрагента из Saby | CRM</title>
  <style>
    :root {
...
<body>
  <div class="container">
    ...
    <div class="glass-card">
      <div style="display:flex; align-items:center; gap:14px; margin-bottom:24px;">
        ...
        <div>
          <h1 style="font-size:24px; font-weight:800; color:var(--text-main);">Новый контрагент</h1>
          <p style="font-size:14px; color:var(--text-secondary); margin-top:2px;">
            Найдите организацию по ИНН или названию, чтобы заполнить данные автоматически
          </p>
        </div>
      </div>
      ...
      <form action="/add_client" method="POST" id="new-client-form">
        ...
        <div class="form-group">
          <label>Поиск организации в Saby/ЕГРЮЛ:</label>
          <input type="text" id="search_input" class="form-control" placeholder="Введите ИНН (например, 7707083893) или наименование компании..." oninput="searchCompany(this.value)" autocomplete="off">
        ...
        <div class="section-title">Контактные лица (Доступы)</div>
  ...
  <script>
    ...
    function searchCompany(val) {
      ...
      box.innerHTML = '<div class="suggest-item" style="color:#94a3b8; text-align:center;">Загрузка...</div>';
        ...
        .then(data => {
          ...
          if (data.contracts && data.contracts.length > 0) {
            data.contracts.forEach(c => {
              ...
              opt.textContent = \`№ \${c.number} от \${c.date} (\${c.status})\`;
              sel.appendChild(opt);
            });
    ...
    function onContractChange() {
      ...
      if (opt && opt.value) {
        document.getElementById('contract_id').value = opt.value;
        document.getElementById('contract_number').value = opt.dataset.number;
      } else {
